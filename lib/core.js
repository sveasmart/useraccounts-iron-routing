...
var ROUTE_PAT = {
    ...
    redirect: Match.Optional(Match.OneOf(String, Match.Where(function (x) { return typeof x === "function" }))),
};
...
AccountsTemplates.configureRoute = function (route, options) {
    ...
    if (this._initialized) {
        throw new Error(
            "Route Configuration can be done only before AccountsTemplates.init!"
        );
    }
    ...
    options = Object.assign({}, this.ROUTE_DEFAULT[route], options || {});
    ...
    if (
        route === "changePwd" &&
        !AccountsTemplates.options.enablePasswordChange
    ) {
        throw new Error(
    ...
    if (
        route === "forgotPwd" &&
        !AccountsTemplates.options.showForgotPasswordLink
    ) {
        throw new Error(
    ...
    if (
        route === "signUp" &&
        AccountsTemplates.options.forbidClientAccountCreation
    ) {
        throw new Error(
    ...
    let path = options.path; // Default provided...
    ...
    if (["enrollAccount", "resetPwd", "verifyEmail"].includes(route)) {
        path += "/:paramToken";
