...
var ensureSignedIn = function () {
    if (!Meteor.userId()) {
        ...
        AccountsTemplates.setState(
            AccountsTemplates.options.defaultState,
            function () {
        ...
        var signInRouteTemplate =
            AccountsTemplates.routes.signIn &&
            AccountsTemplates.routes.signIn.template;
        var template =
        ...
        var defaultLayout =
            AccountsTemplates.options.defaultLayout ||
            Router.options.layoutTemplate;
        var layoutTemplate =
            (options && options.layoutTemplate) || defaultLayout;
...
Iron.Router.plugins.ensureSignedIn = function (router, options) {
    ...
    router.onBeforeAction(ensureSignedIn, options);
...
Router.onStop(function () {
    Tracker.nonreactive(function () {
        ...
        var isKnownRoute = AccountsTemplates.knownRoutes.map(function (path) {
            if (!path) {
                return false;
            }
...
AccountsTemplates.linkClick = function (route) {
    ...
    var firstVisibleInput = this.getFields().find(function (f) {
        return f.visible.includes(route);
    });
...
AccountsTemplates.postSubmitRedirect = function (route) {
    if (AccountsTemplates.avoidRedirect) {
        ...
        var nextPath =
            AccountsTemplates.routes[route] &&
            AccountsTemplates.routes[route].redirect;
        ...
        if (nextPath) {
            ...
            if (
                previousPath &&
                Router.current().route.path() !== previousPath
            ) {
...
AccountsTemplates.submitCallback = function (error, state, onSuccess) {
    ...
    if (error) {
        if (
            ...
            Object.entries(error.details).forEach(function ([fieldId, error]) {
        ...
        if (
            ["enrollAccount", "forgotPwd", "resetPwd", "verifyEmail"].includes(
                state
            )
        ) {
            var redirectTimeout = AccountsTemplates.options.redirectTimeout;
            if (redirectTimeout > 0) {
                AccountsTemplates.timedOutRedirect = Meteor.setTimeout(
                    function () {
                        AccountsTemplates.timedOutRedirect = null;
                        AccountsTemplates.setDisabled(false);
